...
@section('title', 'AngoCar - Acessórios')
@section('content')

<!-- Modal de Detalhes do Acessório -->
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-5">
                        <img id="modalImage" class="img-fluid rounded" style="object-fit: cover; height: 100%; width: 100%;" alt="Acessório">
                    </div>
                    <div class="col-md-7">
        ...
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-10 col-xl-9">
                    ...
                    @forelse ($accessories->chunk(3) as $chunk)
                                    ...
                                    <div class="blog grid-blog h-100 d-flex flex-column shadow-sm">
                                        <div class="blog-image-list overflow-hidden" style="height: 220px;">
                                            <img 
                                                ...
                                                src="{{ $accessory->image ? url('uploads/accessories/' . $accessory->image) : asset('images/default-accessory.jpg') }}" 
                                                alt="{{ $accessory->name }}"
                                                onerror="this.src='{{ asset('images/default-accessory.jpg') }}'"
                                            >
                                        </div>
                                        <div class="blog-content p-3 flex-grow-1 d-flex flex-column">
                                            <h3 class="blog-title h5 mb-2">
                                                <a href="#" class="text-decoration-none text-dark">{{ $accessory->name }}</a>
                                            </h3>
                                            <span class="text-primary fw-bold fs-5">{{ number_format($accessory->price, 2) }} Kz</span>
                                            ...
                                            <button 
                                                ...
                                                data-price="{{ number_format($accessory->price, 2) }} Kz"
                                                data-image="{{ $accessory->image ? asset('uploads/accessories/' . $accessory->image) : asset('images/default-accessory.jpg') }}"
                                                data-description="{{ nl2br(e($accessory->description ?? 'Sem descrição disponível.')) }}"
                                                data-number="{{ $accessory->number ?? '' }}"
                                                data-whatsapp="{{ preg_replace('/\D/', '', $accessory->whatsapp ?? '') }}"
                                                onclick="openAccessoryModal(this)">
                    ...
                    @empty
                       <!--  <div class="text-center py-5">
                            <p class="text-muted">Nenhum acessório disponível no momento.</p>
                        </div> -->
                    @endforelse
...
<!-- Script -->
<script>
function openAccessoryModal(button) {
